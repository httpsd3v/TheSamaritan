...
# Try to import requests, but we have a built-in fallback if it fails
try:
...
def ban_message(rem):
    ...
    return f"You are banned. Try again in {h}h {m}m." if h > 0 else f"You are banned. Try again in {m}m."

def login_required_api(f):
    @wraps(f)
    def decorated(*args, **kwargs):
        uid = session.get("user_id")
        if not uid: return err("Not signed in.", 401)
...
def authenticate(username, password):
    ...
    if not user: raise Exception("Supabase did not return a user object.")
...
def ensure_profile(uid, uname):
    ...
    try: db.table("profiles").upsert({"id": uid, "username": uname}, on_conflict="id").execute()
    except Exception as e: app.logger.error(f"ensure_profile failed: {e}")
...
def get_profile(uid):
    try:
        ...
        p = r.data[0]
        for k, v in [("id", uid), ("username", "unknown"), ("bio", ""), ("verified", False), ("avatar_url", ""), ("banned_until", None)]:
            p.setdefault(k, v)
        return p
    except Exception as e: app.logger.error(f"get_profile failed: {e}"); return None

def username_exists(uname):
    try: return bool(db.table("profiles").select("id").eq("username", uname).limit(1).execute().data)
    except Exception: return False

def count_rows(table, col, val):
    try: return db.table(table).select("id", count="exact").eq(col, val).execute().count or 0
...
def upload_file(f, folder):
    ...
    try:
        try: db.storage.from_(STORAGE_BUCKET).upload(path, data, {"content-type": mime})
        except TypeError: db.storage.from_(STORAGE_BUCKET).upload(path, io.BytesIO(data), {"content-type": mime})
        return db.storage.from_(STORAGE_BUCKET).get_public_url(path)
    except ValueError: raise
    except Exception as e: app.logger.error(f"Storage upload failed: {e}"); raise ValueError(f"Media upload failed: {e}")
...
def serialize_profile(r):
    r = r or {}
    return {"id": r.get("id"), "username": r.get("username", "unknown"), "bio": r.get("bio", ""), "verified": bool(r.get("verified", False)), "avatar_url": r.get("avatar_url", ""), "banned_until": r.get("banned_until")}
...
def serialize_post(r, liked_ids=None, like_counts=None, comment_counts=None):
    ...
    return {"id": pid, "content": r.get("content", ""), "media_url": r.get("image_url") or r.get("media_url") or "", "created_at": r.get("created_at"), "author": author, "likes": like_counts.get(pid, 0), "liked": pid in liked_ids, "comments": comment_counts.get(pid, 0)}

def serialize_comment(r):
    prof = r.get("profiles")
    if isinstance(prof, list): prof = prof[0] if prof else None
    author = serialize_profile(prof) if prof else {"id": None, "username": "deleted", "verified": False, "avatar_url": ""}
    return {"id": r.get("id"), "post_id": r.get("post_id"), "parent_comment_id": r.get("parent_comment_id"), "content": r.get("content", ""), "created_at": r.get("created_at"), "author": author}
...
def create_notification(user_id, actor_id, type_, post_id=None, comment_id=None):
    ...
    try: db.table("notifications").insert({"user_id": user_id, "actor_id": actor_id, "type": type_, "post_id": post_id, "comment_id": comment_id, "read": False}).execute()
...
def notify_user(user_id, title, body, url="/"):
    ...
    try:
        ...
        dead = []
        for s in subs:
            try: webpush({"endpoint": s["endpoint"], "keys": {"p256dh": s["p256dh"], "auth": s["auth"]}}, data=payload, vapid_private_key=VAPID_PRIVATE_KEY, vapid_claims={"sub": VAPID_CLAIMS_EMAIL})
            except WebPushException as e:
                status = getattr(getattr(e, "response", None), "status_code", None)
                app.logger.error(f"webpush failed: {status}")
                if status in (400, 401, 403, 404, 410): dead.append(s.get("id"))
...
# --------------------------------------------------------------------------
# Voice helpers (LiveKit via Twirp + Bulletproof HTTP)
# --------------------------------------------------------------------------
def livekit_api_base():
    url = LIVEKIT_URL.strip()
    if url.startswith("wss://"): url = "https://" + url[6:]
    elif url.startswith("ws://"): url = "http://" + url[5:]
    return url.rstrip("/")

def livekit_client_url():
    url = LIVEKIT_URL.strip()
    if url.startswith("https://"): url = "wss://" + url[8:]
    elif url.startswith("http://"): url = "ws://" + url[7:]
    return url.rstrip("/")

def livekit_service_token(room_list=False):
    now = int(time.time())
    claims = {"iss": LIVEKIT_API_KEY, "sub": "", "nbf": now, "exp": now + 60, "video": {"roomList": True, "roomAdmin": True} if room_list else {}}
    token = jwt.encode(claims, LIVEKIT_API_SECRET, algorithm="HS256")
    return token.decode("utf-8") if isinstance(token, bytes) else token

def make_twirp_request(url, headers, payload):
    """Bulletproof HTTP POST that works even if `requests` library is missing."""
    if _requests:
        try:
            r = _requests.post(url, headers=headers, json=payload, timeout=5)
            return r.status_code, r.json() if r.status_code == 200 else {}
        except Exception as e:
            app.logger.error(f"requests failed: {e}")
            return 500, {}
    else:
        ...
        try:
            ...
            app.logger.error(f"urllib HTTPError: {e.code}")
            return e.code, {}
        except Exception as e:
            app.logger.error(f"urllib Exception: {e}")
            return 500, {}

def get_livekit_rooms_state():
    result = {}
    if not LIVEKIT_URL or not LIVEKIT_API_KEY or not LIVEKIT_API_SECRET: return result

    base = livekit_api_base()
    token = livekit_service_token(room_list=True)
    headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}

    status, data = make_twirp_request(f"{base}/twirp/livekit.RoomService/ListRooms", headers, {})
    if status != 200:
        app.logger.error(f"LiveKit ListRooms failed with status {status}")
        return result

    active_rooms = {room.get("name"): room for room in data.get("rooms", [])}

    for vr in VOICE_ROOMS:
        rid = vr["id"]
        info = active_rooms.get(rid)
        if not info:
            result[rid] = {"count": 0, "identities": []}
            continue

        identities = []
        p_status, p_data = make_twirp_request(f"{base}/twirp/livekit.RoomService/ListParticipants", headers, {"room": rid})
        if p_status == 200:
            for p in p_data.get("participants", []):
                identities.append({"identity": p.get("identity", ""), "name": p.get("name", "")})
        else:
            app.logger.error(f"LiveKit ListParticipants for {rid} failed with status {p_status}")

        result[rid] = {"count": len(identities) or int(info.get("numParticipants", 0)), "identities": identities}
    return result
...
def api_me():
    ...
    if p:
        if get_ban_remaining(p) is not None: session.clear(); return ok(authenticated=False)
    else:
        p = {"id": session["user_id"], "username": session.get("username", "unknown"), "bio": "", "verified": False, "avatar_url": "", "banned_until": None}
        ensure_profile(p["id"], p["username"])
    return ok(authenticated=True, user={"id": session["user_id"], "username": p["username"]}, profile=serialize_profile(p), is_admin=is_admin_username(p["username"]))
...
def api_signin():
    ...
    if prof:
        rem = get_ban_remaining(prof)
        if rem is not None: return err(ban_message(rem), 403)
...
def api_feed():
    ...
    try:
        ...
        if blocked_ids: q = q.not_.in_("user_id", blocked_ids)
...
def api_delete_post(pid):
    try: post = db.table("posts").select("*").eq("id", pid).limit(1).execute().data[0]
    except Exception: return err("Post not found.", 404)
    if post.get("user_id") != session["user_id"] and not is_admin_username(session.get("username", "")): return err("Forbidden.", 403)
    try:
...
def api_comments(pid):
    if request.method == "GET":
        try:
            ...
            out = []
            for c in comments: item = serialize_comment(c); item["post_author_id"] = owner_id; out.append(item)
...
def api_delete_comment(cid):
    try: c = db.table("comments").select("*").eq("id", cid).limit(1).execute().data[0]
    except Exception: return err("Comment not found.", 404)
    try:
        p = db.table("posts").select("user_id").eq("id", c.get("post_id")).limit(1).execute().data
        p_owner = p[0].get("user_id") if p else None
    except Exception: p_owner = None
    if c.get("user_id") != session["user_id"] and p_owner != session["user_id"] and not is_admin_username(session.get("username", "")): return err("Forbidden.", 403)
...
def api_change_password():
    ...
    except Exception: return err("Current password incorrect.", 403)
...
def api_delete_account():
    ...
    except Exception: return err("Incorrect password.", 403)
...
def api_profile(uname):
    ...
    except Exception: return err("User not found.", 404)
    for k, v in [("bio", ""), ("verified", False), ("avatar_url", ""), ("banned_until", None)]: p.setdefault(k, v)
    ...
    return ok(profile=serialize_profile(p), posts=hydrate_posts(posts), followers=followers, following=following, is_me=is_me, is_following=is_following, is_blocked=is_blocked)
...
def api_update_profile():
    ...
    try:
        if new_u != cur_u:
            ...
            except Exception: db.auth.admin.update_user_by_id(uid, {"email": f"{new_u}{DOMAIN}", "user_metadata": {"username": new_u}})
            auth_updated = True
...
def api_toggle_block(uid):
    if uid == session.get("user_id"): return err("Cannot block yourself.")
    try:
        ex = db.table("blocks").select("id").eq("blocker_id", session["user_id"]).eq("blocked_id", uid).limit(1).execute().data
        if ex: db.table("blocks").delete().eq("id", ex[0]["id"]).execute(); return ok(blocked=False)
        else: db.table("blocks").insert({"blocker_id": session["user_id"], "blocked_id": uid}).execute(); return ok(blocked=True)
    except Exception as e: app.logger.error(f"Block failed: {e}"); return err("Could not update block.", 500)
...
def api_block_status(uid):
    ...
    except Exception: return ok(blocked=False)
...
def api_admin_ban(uid):
    ...
    if admin_error: return admin_error
    d = request.get_json(silent=True) or {}
    try: hours = float(d.get("hours", 0))
    except Exception: hours = 0
    banned_until = None if hours <= 0 else (datetime.now(timezone.utc) + timedelta(hours=hours)).isoformat()
    try: db.table("profiles").update({"banned_until": banned_until}).eq("id", uid).execute(); return ok(banned_until=banned_until)
    except Exception as e: app.logger.error(f"Ban failed: {e}"); return err("Could not update ban.", 500)
...
def api_push_subscribe():
    ...
    payload = {"user_id": session["user_id"], "endpoint": ep, "p256dh": keys["p256dh"], "auth": keys["auth"]}
    try:
        existing = db.table("push_subscriptions").select("id").eq("endpoint", ep).limit(1).execute().data
        if existing: db.table("push_subscriptions").update(payload).eq("id", existing[0]["id"]).execute()
        else:
            try: db.table("push_subscriptions").insert(payload).execute()
            except Exception: db.table("push_subscriptions").update(payload).eq("endpoint", ep).execute()
        return ok()
...
def api_notifications():
    try:
        ...
        out = [{"id": n.get("id"), "type": n.get("type"), "post_id": n.get("post_id"), "comment_id": n.get("comment_id"), "read": n.get("read"), "created_at": n.get("created_at"), "actor": actors.get(n.get("actor_id")) or {"username": "someone", "verified": False, "avatar_url": ""}} for n in notifs]
...
def api_unread_count():
    ...
    except Exception: return ok(count=0)
...
def api_voice_rooms():
    ...
    try:
        ...
        app.logger.info(f"Voice room states fetched: {states}") # Explicit logging for debugging
        ...
        if ids:
            ...
            except Exception as e: app.logger.error(f"voice profiles failed: {e}")
        for vr in VOICE_ROOMS:
            st = states.get(vr["id"], {"count": 0, "identities": []})
            parts = [profiles.get(p.get("identity")) or {"id": p.get("identity"), "username": p.get("name") or "user", "verified": False, "avatar_url": ""} for p in st.get("identities", [])]
...
def api_voice_token():
    ...
    if room_id not in {r["id"] for r in VOICE_ROOMS}: return err("Unknown voice room.", 400)
    if not LIVEKIT_URL or not LIVEKIT_API_KEY or not LIVEKIT_API_SECRET: return err("Voice env vars missing in Render.", 500)
    try:
        now = int(time.time())
        claims = {"iss": LIVEKIT_API_KEY, "sub": session["user_id"], "name": session.get("username", "user"), "nbf": now, "exp": now + 600, "video": {"roomJoin": True, "room": room_id, "canPublish": True, "canSubscribe": True}}
        token = jwt.encode(claims, LIVEKIT_API_SECRET, algorithm="HS256")
        if isinstance(token, bytes): token = token.decode("utf-8")
        return ok(token=token, url=livekit_client_url(), room=room_id)
    except Exception as e: app.logger.error(f"Voice token failed: {e}"); return err(f"Token Error: {e}", 500)
