...
def get_livekit_rooms_state(room_ids):
    result = {}
    if not room_ids:
        return result
    ...
    status, data = make_twirp_request(base + "/twirp/livekit.RoomService/ListRooms", headers, {})
    if status != 200:
        app.logger.error(f"LiveKit ListRooms failed with status {status}")
        return result
    active = {room.get("name"): room for room in (data or {}).get("rooms", [])}
    for rid in room_ids:
        info = active.get(rid)
        if not info:
            ...
            continue
        identities = []
        p_status, p_data = make_twirp_request(base + "/twirp/livekit.RoomService/ListParticipants", headers, {"room": rid})
        if p_status == 200:
            for p in (p_data or {}).get("participants", []):
...
# --------------------------------------------------------------------------
# Voice routes (dynamic user-created rooms)
# --------------------------------------------------------------------------
...
def api_voice_rooms():
    rooms = []
    try:
        db_rooms = db.table("voice_rooms").select("*").order("created_at", desc=False).limit(50).execute().data or []
    except Exception as e:
        app.logger.error(f"voice_rooms fetch failed: {e}")
        db_rooms = []

    states = get_livekit_rooms_state([r["id"] for r in db_rooms])
    ...
    for r in db_rooms:
        st = states.get(r["id"], {"count": 0, "identities": []})
        parts = []
...
def api_voice_create():
    ...
    if not name:
        return err("Room name required.")
    rid = uuid.uuid4().hex[:10]
    try:
        db.table("voice_rooms").insert({
            "id": rid,
            "name": name,
        ...
        return err("Could not create room.", 500)


@app.route("/api/voice/delete/<rid>", methods=["POST"])
@login_required_api
def api_voice_delete(rid):
    try:
        r = db.table("voice_rooms").select("*").eq("id", rid).limit(1).execute().data
        room = r[0] if r else None
        if not room:
            return ok()
        if room.get("created_by") != session["user_id"] and not is_admin_username(session.get("username", "")):
            return err("Only the room creator can delete it.", 403)
        db.table("voice_rooms").delete().eq("id", rid).execute()
        return ok()
    except Exception as e:
        app.logger.error(f"voice delete failed: {e}")
        return err("Could not delete room.", 500)


@app.route("/api/voice/cleanup/<rid>", methods=["POST"])
@login_required_api
def api_voice_cleanup(rid):
    try:
        st = get_livekit_rooms_state([rid]).get(rid, {"count": 0})
        if st.get("count", 0) == 0:
            db.table("voice_rooms").delete().eq("id", rid).execute()
    except Exception as e:
        app.logger.error(f"voice cleanup failed: {e}")
    return ok()
...
def api_voice_token():
    ...
    try:
        r = db.table("voice_rooms").select("id").eq("id", rid).limit(1).execute().data
        if not r:
            return err("Room no longer exists.", 404)
    except Exception:
        return err("Room no longer exists.", 404)
    if not LIVEKIT_URL or not LIVEKIT_API_KEY or not LIVEKIT_API_SECRET:
    ...
    try:
        ...
        claims = {
            ...
            "nbf": now,
            "exp": now + 600,
            "video": {"roomJoin": True, "room": rid, "canPublish": True, "canSubscribe": True},
        }
        token = jwt.encode(claims, LIVEKIT_API_SECRET, algorithm="HS256")
        if isinstance(token, bytes):
            token = token.decode("utf-8")
        return ok(token=token, url=livekit_client_url(), room=rid)
    except Exception as e:
